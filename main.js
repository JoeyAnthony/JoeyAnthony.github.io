...
var numCubes = 8;
var renderCubes = document.currentScript.getAttribute("cubes");
...
`
	...
	vec3 wave(vec2 st, float wavewidth, float waveheight, float heightposwave, float widthposwave, vec3 uppercolour, vec3 lowercolour, float time){
		return 	step(st.y, ((sin(st.x*wavewidth+time +widthposwave)+heightposwave)/waveheight))	/*<- upperwave*/ * mix( ((sin(st.x*wavewidth+time +widthposwave)+heightposwave)/waveheight) +0.816, sin(st.y)*5.008, .5)*uppercolour*1.2 /*<-upperwave colour interpolation*/ 
	...
	void main() {
		vec2 st = vec2(gl_FragCoord.x/1000., gl_FragCoord.y/u_resolution.y*1.5);//800 is a nice value so no stretch
		...
		vec3 w4 = wave(st, 1.3, 5.5, 2.25,1.276, vec3(0.535,0.654,0.680), bg, u_time);
		vec3 w5 = wave(st, 1.3, 5.5, 3.754,3.316, vec3(0.348,0.366,0.985), bg, u_time/1.35);
